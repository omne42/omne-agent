...
- 内置 role prompts：`prompt/roles/*.md`（含权限 frontmatter）编译期嵌入；每 turn 以 `@role <role>...</role>` 注入到最新 user message；当 `prompt_cache=false`（provider capability 或 per-model 覆盖）时同时写入 system prompt。
