...
- `githooks/pre-commit`：新增 Rust 单文件行数 gate：任意暂存的 `.rs` 文件超过 1000 行将拒绝提交（要求拆分模块/降低复杂度）。
