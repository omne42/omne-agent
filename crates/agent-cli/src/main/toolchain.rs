...
async fn run_toolchain_bootstrap(args: ToolchainBootstrapArgs) -> anyhow::Result<()> {
    let result = bootstrap_toolchain(&args).await?;
    if args.json {
...
async fn bootstrap_toolchain(args: &ToolchainBootstrapArgs) -> anyhow::Result<ToolchainBootstrapResult> {
...
async fn bootstrap_one_tool(
    ...
    if has_public_recipe(tool, target_triple) {
        ...
        detail_parts.push(format!("no public bootstrap recipe for target `{target_triple}`"));
    }
...
fn has_public_recipe(tool: &str, target_triple: &str) -> bool {
    match tool {
        ...
        "git" => target_triple == "x86_64-pc-windows-msvc" || target_triple == "aarch64-pc-windows-msvc",
        _ => false,
    }
}
...
async fn install_from_public(
    ...
    match tool {
        "gh" => install_gh_from_public(target_triple, binary_ext, destination, cfg, http_client).await,
...
async fn install_gh_from_public(
    ...
    let (bytes, source_url) =
        download_with_candidates(http_client, &asset.browser_download_url, &cfg.mirror_prefixes)
            .await?;
...
async fn install_git_from_public(
    ...
    let asset = select_mingit_asset_for_target(&release.assets, target_triple).ok_or_else(|| {
        anyhow::anyhow!("cannot find MinGit asset for target `{target_triple}`")
    })?;
    let expected_sha = parse_sha256_digest(asset.digest.as_deref())
        .ok_or_else(|| anyhow::anyhow!("missing sha256 digest in git-for-windows release metadata"))?;
    let (bytes, source_url) =
        download_with_candidates(http_client, &asset.browser_download_url, &cfg.mirror_prefixes)
            .await?;
    verify_sha256(&bytes, &expected_sha)?;
...
fn select_mingit_asset_for_target<'a>(
    ...
    match target_triple {
        "x86_64-pc-windows-msvc" => assets
            .iter()
            .find(|asset| asset.name.starts_with("MinGit-") && asset.name.ends_with("-busybox-64-bit.zip"))
            .or_else(|| {
...
fn install_from_zip(
    ...
    let mut archive =
        zip::ZipArchive::new(Cursor::new(content)).context("open zip archive for tool bootstrap")?;
...
mod toolchain_tests {
    ...
    async fn public_gh_install_from_mock_release_api() -> anyhow::Result<()> {
        ...
        routes.insert("/api/repos/cli/cli/releases/latest".to_string(), release_body);
        ...
        let source = install_gh_from_public(
