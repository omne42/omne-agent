...
async fn run_workflow_fan_out(
    ...
    let concurrency_limit = scheduling.effective_concurrency_limit;
    ...
    let mut ready_wait_rounds = BTreeMap::<String, usize>::new();
    let mut task_statuses = BTreeMap::<String, TurnStatus>::new();
    ...
    while finished.len() < tasks.len() {
        update_ready_wait_rounds(tasks, &started, &task_statuses, &mut ready_wait_rounds);
        while active.len() < concurrency_limit {
            ...
            ready_wait_rounds.remove(&task.id);

            let forked = if subagent_fork {
        ...
        while idx < active.len() {
                    ...
                    match event.kind {
                        ...
                        omne_protocol::ThreadEventKind::ApprovalRequested {
                            ...
                            approval_issue = Some(FanOutApprovalIssue {
                                task_id: active[idx].task_id.clone(),
                                thread_id,
                                turn_id,
                                approval_id,
                                action,
            ...
            if let Some(issue) = approval_issue {
                ...
                task_statuses.insert(task.task_id.clone(), TurnStatus::Interrupted);
                finished.push(pending_approval_task_result(
                    task.task_id.clone(),
                    task.title.clone(),
                    task.thread_id,
                    task.turn_id,
            ...
            if let Some((status, reason)) = done {
                let task = active.remove(idx);
                task_statuses.insert(task.task_id.clone(), status);
        ...
        if finished.len() < tasks.len() {
            let blocked = collect_dependency_blocked_task_ids(tasks, &started, &task_statuses);
            if !blocked.is_empty() {
                for (task_id, dep_id, dep_status) in blocked {
                    started.insert(task_id.clone());
                    ready_wait_rounds.remove(&task_id);
                    task_statuses.insert(task_id.clone(), TurnStatus::Cancelled);
                    if let Some(task) = tasks.iter().find(|task| task.id == task_id) {
                        finished.push(blocked_task_result(task, &dep_id, dep_status));
                    }
                }
            }
            if active.is_empty() {
