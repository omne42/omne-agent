    impl UiState {
        fn handle_mouse(&mut self, mouse: MouseEvent) -> bool {
            ...
            let (scroll_up, scroll_down) = match mouse.kind {
            ...
            if let Some(overlay) = self.overlays.last_mut() {
                match overlay {
                    Overlay::Text(view) => {
                        ...
                        view.scroll = if scroll_up {
                            view.scroll.saturating_sub(SCROLL_LINES)
                        } else {
                            view.scroll.saturating_add(SCROLL_LINES)
                        };
                        return view.scroll != before;
                    }
                    Overlay::CommandPalette(view) => {
                        ...
                        if scroll_up {
                            ...
                            view.selected = (view.selected + step).min(view.filtered.len() - 1);
                        }
                        return view.selected != before;
                    }
                    Overlay::Approvals(view) => {
                        ...
                        if scroll_up {
                            view.selected = view.selected.saturating_sub(step);
                        } else if !view.approvals.is_empty() {
                            view.selected = (view.selected + step).min(view.approvals.len() - 1);
                        }
                        return view.selected != before;
                    }
                    Overlay::Processes(view) => {
                        ...
                        if scroll_up {
                            ...
                            view.selected = (view.selected + step).min(view.processes.len() - 1);
                        }
                        return view.selected != before;
                    }
                    Overlay::Artifacts(view) => {
                        ...
                        if scroll_up {
                            ...
                            view.selected = (view.selected + step).min(view.artifacts.len() - 1);
                        }
                        return view.selected != before;
                    }
                }
            ...
            match self.active_thread {
                None => {
                    let before = self.selected_thread;
                    let step = 1usize;
                    if scroll_up {
                        self.selected_thread = self.selected_thread.saturating_sub(step);
                    } else if !self.threads.is_empty() {
                        self.selected_thread =
                            (self.selected_thread + step).min(self.threads.len() - 1);
                    }
                    self.selected_thread != before
                }
                Some(_) => {
                    if self.inline_palette.is_some() {
                        let before = self
                            .inline_palette
                            .as_ref()
                            .map(|inline| inline.view.selected)
                            .unwrap_or_default();
                        self.move_inline_selection(if scroll_up { -1 } else { 1 });
                        let after = self
                            .inline_palette
                            .as_ref()
                            .map(|inline| inline.view.selected)
                    ...
                    let before_scroll = self.transcript_scroll;
                    let before_follow = self.transcript_follow;

                    self.transcript_follow = false;
                    if scroll_up {
                        self.transcript_scroll =
                            self.transcript_scroll.saturating_sub(SCROLL_LINES);
                    } else {
                        self.transcript_scroll =
                            self.transcript_scroll.saturating_add(SCROLL_LINES);
                    ...
                    self.transcript_scroll != before_scroll || self.transcript_follow != before_follow
                }
            }
        }
