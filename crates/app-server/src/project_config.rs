...
pub enum ProjectConfigSource {
    /// `.omne_agent_data/config_local.toml` (gitignored)
    Local,
    /// `.omne_agent_data/config.toml` (commit-safe)
    Shared,
}
...
fn parse_dotenv_openai(contents: &str) -> DotenvOpenAiOverrides {
    ...
    out.provider = out.dotenv.get("OMNE_AGENT_OPENAI_PROVIDER").cloned();
    out.base_url = out.dotenv.get("OMNE_AGENT_OPENAI_BASE_URL").cloned();
    out.model = out.dotenv.get("OMNE_AGENT_OPENAI_MODEL").cloned();
    out.fallback_providers = out
        .dotenv
        .get("OMNE_AGENT_OPENAI_FALLBACK_PROVIDERS")
...
pub async fn load_project_config(thread_root: &Path) -> LoadedProjectConfig {
    let omne_agent_data_dir = thread_root.join(".omne_agent_data");
    let config_local_toml_path = omne_agent_data_dir.join("config_local.toml");
    let config_toml_path = omne_agent_data_dir.join("config.toml");
    let env_path = omne_agent_data_dir.join(".env");
...
mod tests {
    ...
    async fn loads_provider_reasoning_and_auth_command_from_config_toml() -> anyhow::Result<()> {
        ...
        let root = tmp.path();
        let omne_agent_data = root.join(".omne_agent_data");
        tokio::fs::create_dir_all(&omne_agent_data).await?;

        tokio::fs::write(
            omne_agent_data.join("config.toml"),
            r#"
[project_config]
enabled = true
...
"#,
        ...
        .await?;
        tokio::fs::write(
            omne_agent_data.join(".env"),
            r#"
OMNE_AGENT_OPENAI_PROVIDER=openai-auth-command
"#,
        )
        .await?;
...
"#,
        ...
        tokio::fs::write(
            omne_agent_data.join(".env"),
            r#"
OMNE_AGENT_OPENAI_BASE_URL=https://env.example/v1
"#,
        )
        .await?;
...
"#,
        ...
        tokio::fs::write(
            omne_agent_data.join(".env"),
            r#"
OMNE_AGENT_OPENAI_FALLBACK_PROVIDERS=openai-codex-apikey, openai-auth-command
"#,
        )
        .await?;
