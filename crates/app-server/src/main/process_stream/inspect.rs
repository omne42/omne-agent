async fn handle_process_inspect(server: &Server, params: ProcessInspectParams) -> anyhow::Result<Value> {
    ...
    let stdout_tail =
        omne_agent_core::redact_text(&tail_file_lines(PathBuf::from(&info.stdout_path), max_lines).await?);
    let stderr_tail =
        omne_agent_core::redact_text(&tail_file_lines(PathBuf::from(&info.stderr_path), max_lines).await?);
