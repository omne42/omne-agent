use omne_agent_protocol::{ThreadEventKind, ToolId, ToolStatus};
...
fn hooks_process_timeout() -> Duration {
    let value = std::env::var("OMNE_AGENT_HOOK_PROCESS_TIMEOUT_SECS")
        .ok()
...
fn redact_and_truncate_string(s: &str, max_chars: usize) -> String {
    truncate_owned_chars(omne_agent_core::redact_text(s), max_chars)
}
...
async fn run_hook_commands(
    ...
    for command in commands {
        ...
        env.insert(
            "OMNE_AGENT_HOOK_INPUT_PATH".to_string(),
            input_path.display().to_string(),
        );
        env.insert(
            "OMNE_AGENT_HOOK_OUTPUT_PATH".to_string(),
            output_path.display().to_string(),
        );
        env.insert(
            "OMNE_AGENT_HOOK_POINT".to_string(),
            hook_point.as_str().to_string(),
        );
        if let Some(tool) = ctx.tool {
            env.insert("OMNE_AGENT_HOOK_TOOL".to_string(), tool.to_string());
        }
