...
mod repo_index_search_tests {
    ...
    async fn repo_search_writes_artifact_and_skips_secrets() -> anyhow::Result<()> {
        ...
        tokio::fs::create_dir_all(repo_dir.join(".omne_agent_data/tmp")).await?;
        tokio::fs::write(repo_dir.join(".omne_agent_data/tmp/leak.txt"), "needle\n").await?;
        ...
        assert!(!text.contains(".omne_agent_data/tmp"));

        Ok(())
    }

    #[tokio::test]
    async fn repo_index_writes_artifact_and_ignores_omne_agent_data() -> anyhow::Result<()> {
        let tmp = tempfile::tempdir()?;
        let repo_dir = tmp.path().join("repo");
        ...
        tokio::fs::create_dir_all(repo_dir.join(".omne_agent_data/tmp")).await?;
        tokio::fs::write(
            repo_dir.join(".omne_agent_data/tmp/leak.txt"),
...
"#,
        ...
        handle_thread_configure(
            ...
            ThreadConfigureParams {
                thread_id,
                approval_policy: Some(omne_agent_protocol::ApprovalPolicy::Manual),
                sandbox_policy: None,
