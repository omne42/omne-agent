use omne_agent_eventlog::ThreadState;
...
async fn handle_thread_attention(
    ...
    let mut decided = HashSet::<omne_agent_protocol::ApprovalId>::new();
    ...
    for event in &events {
        ...
        match &event.kind {
            ...
            omne_agent_protocol::ThreadEventKind::ApprovalDecided { approval_id, .. } => {
    ...
    let attention_state = if !pending_approvals.is_empty() {
        ...
        match last_turn_status {
            Some(omne_agent_protocol::TurnStatus::Completed) => "done",
            Some(omne_agent_protocol::TurnStatus::Interrupted) => "interrupted",
            Some(omne_agent_protocol::TurnStatus::Failed) => "failed",
            Some(omne_agent_protocol::TurnStatus::Cancelled) => "cancelled",
            Some(omne_agent_protocol::TurnStatus::Stuck) => "stuck",
            None => "idle",
        }
    };
...
async fn build_stuck_report_markdown(
    ...
    let mut last_approval_in_turn: Option<(omne_agent_protocol::ApprovalId, String)> = None;
    let mut last_approval_any: Option<(omne_agent_protocol::ApprovalId, String)> = None;
    let mut last_tool_in_turn: Option<(omne_agent_protocol::ToolId, String)> = None;
    let mut last_tool_any: Option<(omne_agent_protocol::ToolId, String)> = None;
    ...
    for event in &events {
        match &event.kind {
            ...
            omne_agent_protocol::ThreadEventKind::ProcessExited { process_id, .. } => {
    ...
    md.push_str("\n## Next actions\n");
    md.push_str(&format!("- omne-agent thread attention {thread_id}\n"));
    md.push_str(&format!("- omne-agent approval list {thread_id}\n"));
    md.push_str(&format!("- omne-agent process list --thread-id {thread_id}\n"));
    if let Some(process) = &process {
        md.push_str(&format!("- omne-agent process tail {}\n", process.process_id));
    }
...
async fn handle_thread_list_meta(
    ...
    for thread_id in thread_ids {
        ...
        let first_message = events.iter().find_map(|event| match &event.kind {
            omne_agent_protocol::ThreadEventKind::TurnStarted { input, .. } => Some(input.clone()),
            _ => None,
        });
        let first_message = first_message
            .map(|text| truncate_chars(&omne_agent_core::redact_text(text.trim()), 500))
