    ...
    tokio::task::spawn_blocking(move || -> anyhow::Result<()> {
            ...
            .filter_entry(|entry| should_walk_isolated_workspace_entry(&source_root, entry))
        {
            ...
            if entry.file_type().is_symlink() {
                anyhow::bail!(
                    "isolated workspace copy refuses symlink: {}",
                    rel.display()
                );
            }
...
mod subagents_runtime_artifacts_tests {
    ...
    #[cfg(unix)]
    #[tokio::test]
    async fn isolated_workspace_copy_rejects_symlinks() -> anyhow::Result<()> {
        ...
        std::os::unix::fs::symlink(&outside, source_root.join("link.txt"))?;

        let err = copy_workspace_into_isolated_root(&source_root, &isolated_root, 1024, 4096)
            .await
            .expect_err("copy backend should reject symlinks");
        let message = err.to_string();
        assert!(message.contains("refuses symlink"));
        assert!(message.contains("link.txt"));
        assert!(!isolated_root.join("link.txt").exists());
        Ok(())
    }
}
