...
async fn active_subagent_threads(
    server: &super::Server,
    parent_thread_id: omne_agent_protocol::ThreadId,
) -> anyhow::Result<Vec<omne_agent_protocol::ThreadId>> {
    let events = server
        .thread_store
    ...
    let mut spawned_tool_ids = std::collections::HashSet::<omne_agent_protocol::ToolId>::new();
    for event in &events {
        if let omne_agent_protocol::ThreadEventKind::ToolStarted { tool_id, tool, .. } = &event.kind
            && tool == "subagent/spawn"
        {
    ...
    let mut spawned_threads = std::collections::BTreeSet::<omne_agent_protocol::ThreadId>::new();
    for event in &events {
        let omne_agent_protocol::ThreadEventKind::ToolCompleted {
            tool_id,
            status,
            result,
            ..
        ...
        if !matches!(status, omne_agent_protocol::ToolStatus::Completed) {
            continue;
        }
        ...
        let mut record_thread_id = |thread_id: &str| {
            if let Ok(thread_id) = thread_id.parse::<omne_agent_protocol::ThreadId>() {
