...
function detectTargetTriple() {
  const override = process.env.OMNE_AGENT_TARGET_TRIPLE;
...
function resolveVendoredBinary(binName) {
  ...
  const candidate = path.join(pkgRoot, "vendor", targetTriple, "omne-agent", `${binName}${ext}`);
...
function spawnAndExit(bin, args) {
  ...
  child.on("error", (err) => {
    if (err && err.code === "ENOENT") {
      ...
      if (bin === "omne-agent-app-server") {
        usageError(
          "  - Or set OMNE_AGENT_APP_SERVER_BIN to an absolute path (e.g. target/debug/omne-agent-app-server)"
        );
      } else if (bin === "omne-agent") {
        usageError("  - Or set OMNE_AGENT_BIN to an absolute path (e.g. target/debug/omne-agent)");
      }
      process.exit(1);
    }
...
function main() {
  ...
  if (isAppServer) {
    const bin = resolveBinary("omne-agent-app-server", "OMNE_AGENT_APP_SERVER_BIN");
    spawnAndExit(bin, argv.slice(1));
    return;
  }

  const bin = resolveBinary("omne-agent", "OMNE_AGENT_BIN");
