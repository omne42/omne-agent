...
export class JsonRpcStdioClient extends EventEmitter {
  ...
  constructor(child) {
    ...
    child.on("exit", (code, signal) => {
      ...
      const err =
        signal != null
          ? new Error(`omne-agent-app-server exited via signal: ${signal}`)
          : new Error(`omne-agent-app-server exited with code: ${code ?? "unknown"}`);
  ...
  static spawnOmneAgentAppServer({
    bin = process.env.OMNE_AGENT_APP_SERVER_BIN || "omne-agent-app-server",
    args = [],
    env = {},
  } = {}) {
