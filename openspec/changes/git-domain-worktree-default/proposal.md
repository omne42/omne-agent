...
- 将 worktree 创建能力收敛到 `omne-git-runtime`（Git 领域）。
...
- 行为要求：
  - Git 仓库场景默认使用 worktree。
  - 非 Git 仓库或 worktree 失败场景自动回退 copy，且不中断 subagent 流程。
- 架构边界要求：
  - `app-server` 不承载 Git 过程实现细节（不直接执行 Git 子进程来实现领域逻辑）。
  - Git 领域能力由 `omne-git-runtime` 统一提供。
- 可验证检查：
  ...
  - 边界检查（当前阶段）：`rg -n \"Command::new\\(\\\"git\\\"\\)\" crates/app-server/src/agent/tools/dispatch/subagents_runtime_artifacts.rs` 应无结果。
