...
构建一个“通用可复用的安装基建能力”（release / system package / pip），让调用方自行决定安装对象；`omne-agent` 仅把该基建用于 `git`/`gh` 工作树场景，并满足公共资源来源、网络可达性与安全边界要求。

## 做什么

- 在 `p/` 下创建独立仓库（后续命名为 `toolchain-installer`）作为通用辅助安装工具。
- `omne-agent` 不再直接承载复杂下载/镜像路由逻辑，改为作为一个调用方使用该工具。
- 安装器提供通用 `plan` 执行能力，调用方决定“安装什么”。
...
- release/apt/pip 这些安装基建是跨项目能力，应由独立仓库复用。
