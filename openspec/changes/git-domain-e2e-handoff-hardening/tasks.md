...
- [ ] 主链路 E2E：`isolated_write -> handoff -> auto-apply -> archive/delete cleanup`。
- [ ] 策略分支 E2E：`auto` 成功、`auto` 回退、`worktree` 强制失败、`copy` 强制执行。
- [ ] 异常场景 E2E：patch conflict、无 patch、cleanup best-effort 失败。
- [ ] E2E 结果输出统一结构化摘要，便于回归对比。
...
- [ ] E2E 套件可稳定复跑并覆盖主链路 + 关键分支。
- [ ] 交接模板完整，下一位无需聊天上下文可直接继续。
- [ ] Git 领域边界无回流，文档与代码状态一致。
