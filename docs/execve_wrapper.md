...
- `sandbox_network_access=deny` 的当前实现应保持 fail-closed：
  - 已知网络客户端必须拒绝
  - generic launcher 执行代码必须拒绝
  - 路径形式的 opaque 可执行文件也必须拒绝，因为它们的网络行为无法在 wrapper 外可靠证明为安全
- 上述拒绝链路仍然是基于 `argv` 的 best-effort 分类，不是内核级网络命名空间隔离；真正的强隔离需要未来独立的 sandbox primitive。
