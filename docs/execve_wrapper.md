...
> - execve wrapper 二进制：`crates/execve-wrapper`（`pm-execve-wrapper`）
> - 启用方式（最小）：`CODE_PM_EXECVE_WRAPPER=pm-execve-wrapper` + 用 patched bash 运行 `process/start`（见下文）。
...
- 当 `process/start` 启动 `bash` 且设置了 `CODE_PM_EXECVE_WRAPPER` 时，server 会启动一个 per-process 的 execve gate（unix socket, `0600`），并向 bash 注入：
  - `BASH_EXEC_WRAPPER=<CODE_PM_EXECVE_WRAPPER>`
  - `CODE_PM_EXECVE_SOCKET=<.../execve-gate.sock>`
  - `CODE_PM_EXECVE_TOKEN=<random nonce>`
  - `CODE_PM_THREAD_ID=<thread_id>`
  - `CODE_PM_TURN_ID=<turn_id>`（如有）
- patched bash 在每次 `execve` 前会改写 argv：`[wrapper, orig_command, <original argv...>]`；`pm-execve-wrapper` 通过 gate 做裁决后再 `execve` 原命令。
...
- `escalate`：必须进入 approvals（`ApprovalRequested` 落盘）；wrapper 需调用 `codepm.execve.wait` 等待最终 `run/deny`。

### 3.2 `codepm.execve.wait`
...
- execpolicy 对 `git` 设为 `prompt_strict`（或等价规则）后，在 bash 中执行 `git status`：
  - 必须产生 `ApprovalRequested`，`pm inbox` 可见；批准后继续执行。
