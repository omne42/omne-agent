...
> - execve wrapper 二进制：`crates/execve-wrapper`（`omne-agent-execve-wrapper`）
> - 启用方式（最小）：`OMNE_AGENT_EXECVE_WRAPPER=omne-agent-execve-wrapper` + 用 patched bash 运行 `process/start`（见下文）。
...
- 当 `process/start` 启动 `bash` 且设置了 `OMNE_AGENT_EXECVE_WRAPPER` 时，server 会启动一个 per-process 的 execve gate（unix socket, `0600`），并向 bash 注入：
  - `BASH_EXEC_WRAPPER=<OMNE_AGENT_EXECVE_WRAPPER>`
  - `OMNE_AGENT_EXECVE_SOCKET=<.../execve-gate.sock>`
  - `OMNE_AGENT_EXECVE_TOKEN=<random nonce>`
  - `OMNE_AGENT_THREAD_ID=<thread_id>`
  - `OMNE_AGENT_TURN_ID=<turn_id>`（如有）
- patched bash 在每次 `execve` 前会改写 argv：`[wrapper, orig_command, <original argv...>]`；`omne-agent-execve-wrapper` 通过 gate 做裁决后再 `execve` 原命令。
...
- `OMNE_AGENT_THREAD_ID`
- `OMNE_AGENT_TURN_ID`
- `OMNE_AGENT_EXECVE_SOCKET`（或 stdio 连接信息）
- `OMNE_AGENT_EXECVE_TOKEN`（随机 nonce；用于本地鉴权，防止其它进程伪造请求）
...
- `escalate`：必须进入 approvals（`ApprovalRequested` 落盘）；wrapper 需调用 `omne_agent.execve.wait` 等待最终 `run/deny`。

### 3.2 `omne_agent.execve.wait`
...
- wrapper 与 decision service 的连接必须是本机可信通道：
  ...
  - 用 `OMNE_AGENT_EXECVE_TOKEN` 做一次请求鉴权（否则其它本地进程可伪造 allow）
...
- 在启用 wrapper 的 bash 中执行 `curl https://example.com`（默认 `sandbox_network_access=deny`）：
  - 必须被拒绝，stderr 含拒绝原因，且不产生审批事件。
- execpolicy 对 `git` 设为 `prompt_strict`（或等价规则）后，在 bash 中执行 `git status`：
  - 必须产生 `ApprovalRequested`，`omne-agent inbox` 可见；批准后继续执行。
