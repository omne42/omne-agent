# OmneAgent vNext 使用流程（RTS 风格 / 目标态）
...
1. **创建 workspace**
   - 系统创建隔离目录（当前 `isolated_write` 默认是 `git worktree` 优先，失败回退 copy；**不会启动本地 Git 服务**；`git/workspace` 不以 Docker 为前提，但不禁止 agent 自己运行 Docker）。
