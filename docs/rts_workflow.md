...
1. **创建 workspace**
   - 系统创建隔离目录（本地 `/tmp` 或 git worktree/容器是实现细节）。
...
7. **Archive（回收）**
   - 执行 `archive/teardown` 脚本回收外部资源（容器/DB branch/端口占用）。
...
1. **workspace 生命周期脚本化**：并行开发真正的冲突点在外部资源（端口/DB/容器/缓存），必须靠 `setup/run/teardown` 自动化解决（参考 `docs/research/superset.md`）。
