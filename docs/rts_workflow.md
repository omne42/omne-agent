# omne-agent vNext 使用流程（RTS 风格 / 目标态）
...
>
> v0.2.0 现状补充：workspace hooks 已支持最小执行入口（`.omne_agent_data/spec/workspace.yaml` + `omne-agent thread hook-run setup|run|archive`），但还没有“自动在创建/归档时触发”的编排逻辑（见 `docs/workspace_hooks.md`）。
