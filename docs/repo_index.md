...
- 默认忽略目录：`.git`、`target`、`node_modules`、`example`（实现对照：`crates/app-server/src/main/file_read_glob_grep.rs`）。
- 并额外跳过 `.omne_agent_data/{tmp,threads,locks,logs,data,repos,reference}/`（避免扫描运行时目录）。
- 参数边界：
...
- 忽略规则与 `repo/index`/`repo/search` 对齐（跳过 `.env`、`.omne_agent_data/*` 等）。
...
- `omne-agent artifact list/read/delete` 能对 `repo_search` 产物进行管理（见 `docs/artifacts.md`）。
- 产物 metadata 的 provenance 能定位到 thread/turn/tool（见 `omne_agent_protocol::ArtifactProvenance`）。
...
```bash
omne-agent repo search <thread_id> "TODO" --include-glob "crates/**" --max-matches 50 --json
omne-agent repo index <thread_id> --include-glob "**/*" --max-files 20000 --json
omne-agent repo symbols <thread_id> --include-glob "crates/**" --max-files 20000 --max-symbols 5000 --json
```

验收点：

- `repo/search` 必须返回 `artifact_id`，并且 `omne-agent artifact read` 能读到固定模板的 Markdown。
- `repo/index` 生成的 artifact 必须标记是否截断（避免“看起来像全量，其实不是”）。
- `repo/symbols` 必须返回 `artifact_id`，并且 `omne-agent artifact read` 能看到 `# Repo Symbols (Rust)` 与符号列表；若超限，必须显式标记 `truncated=true`。
