...
>
> 状态：v0.2.0 已实现 **P0（目录快照）**：`thread/checkpoint/{create,list,restore}` + CLI `omne-agent checkpoint {create,list,restore}`。
...
```
<agent_root>/threads/<thread_id>/artifacts/checkpoints/<checkpoint_id>/
  manifest.json
...
- 必须排除运行时目录：
  - `.omne_agent_data/tmp/**`、`.omne_agent_data/threads/**`
  - `.omne_agent_data/data/**`、`.omne_agent_data/repos/**`
  - `.omne_agent_data/reference/**`
  - `.omne_agent_data/locks/**`、`.omne_agent_data/logs/**`
...
- omne-agent process kill <process_id>
- omne-agent checkpoint restore <thread_id> <checkpoint_id>
```
...
- 受 sandbox/mode 控制（不能把 `.omne_agent_data/{tmp,threads,data,repos,locks,logs}/**` 当可回滚对象）。
...
```bash
omne-agent checkpoint create <thread_id> --label "before refactor"
omne-agent checkpoint list <thread_id>
omne-agent checkpoint restore <thread_id> <checkpoint_id>
```
...
# 2) 人工决定
omne-agent approval decide <thread_id> <approval_id> --approve

# 3) 带 approval_id 重试，执行实际 restore
omne-agent checkpoint restore <thread_id> <checkpoint_id> --approval-id <approval_id>
```
