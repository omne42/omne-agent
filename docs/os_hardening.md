...
>
> 状态：v0.2.x 已实现 **最小子集**（`OMNE_AGENT_HARDENING` 开关 + pre-main best-effort + child process 最小 env scrub/非交互）；其余仍为 TODO（见下文）。
...
- env：`OMNE_AGENT_HARDENING=off|best_effort`
  - 默认：`best_effort`
...
- Windows：当前未支持（`omne-agent-app-server` 依赖 `nix`）。
...
- `omne-agent-app-server` 启动日志中能看到：增强项是否启用、哪些项成功应用、哪些项因平台/权限失败并被记录为 warning。
