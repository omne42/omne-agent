...
## 在 omne-agent 仓库中的定位（v0.2.x 现状）

> 本仓库依赖的 `ditto-llm`（本地 checkout：`../ditto-llm`）目前包含两层能力：
>
> 1. omne-agent v0.2.x 当前用到的“路由/配置层”：provider profile 配置 + OpenAI-compatible `/models` 发现 + `thinking`(reasoning.effort) 配置。
...
>
> omne-agent 主程序已在 `omne-agent-app-server` agent loop 中接入 `ditto-llm::LanguageModel`（默认 OpenAI Responses；可通过 provider capabilities 切换到 OpenAI-compatible Chat Completions）。`crates/openai` 目前仍保留用于 legacy types / SSE 解析等。
