...
- `./.omne_agent_data/spec/workspace.yaml`
- `./.omne_agent_data/spec/workspace.yml`

> 注意：这是项目配置（可提交）。运行时数据根目录也是 `./.omne_agent_data/`；不要把 hooks 配置写进 `threads/`、`tmp/` 等运行时目录。
...
```bash
omne-agent thread hook-run <thread_id> setup
omne-agent thread hook-run <thread_id> run
omne-agent thread hook-run <thread_id> archive
```
...
- 需要审批：返回 `{ needs_approval: true, approval_id, thread_id, hook }`
  - CLI 会报错并提示：先 `omne-agent approval decide <thread_id> <approval_id> --approve`，然后带 `--approval-id <id>` 重跑。
...
omne-agent thread start --cwd . --json

# 运行 setup hook（需要你已经创建 `.omne_agent_data/spec/workspace.yaml`）
omne-agent thread hook-run <thread_id> setup --json
```
