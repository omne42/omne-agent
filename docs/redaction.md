...
对 `mcp/call` 来说还有一层额外保护：

- 当返回值超过 `mcp_result` artifact 阈值并需要写盘时，会先对 JSON 做结构化脱敏。
- 敏感 key（如 `token`、`authorization`、`api_key`）对应的值会直接替换成 `"<REDACTED>"`。
- 其它 string 值仍会经过 `redact_text()`，以覆盖 Bearer token、已知 key 形态等文本泄漏。

对照实现：

- `crates/app-server/src/main/artifact.rs`（写入前 `omne_core::redact_text(&params.text)`）
- `crates/app-server/src/main/mcp/runtime.rs`（`mcp_result` artifact 写盘前的 JSON 脱敏）
...
- `crates/app-server/src/main/preamble.rs`（`CHILD_PROCESS_ENV_SCRUB_KEYS` + `scrub_child_process_env()`）
...
rg -n \"CHILD_PROCESS_ENV_SCRUB_KEYS\" crates/app-server/src/main/preamble.rs
```
