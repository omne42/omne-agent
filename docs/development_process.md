# omne-agent vNext 重新开发流程（Agent-First）
...
2. **注意力队列（Attention / Inbox）**：把“需要人介入”的点变成可枚举状态。v0.2.0 已实现的 `attention_state` 见 `docs/attention.md`；`plan_ready/diff_ready/test_failed` 属于语义标记（markers，TODO），用于减少用户解析 stdout 的成本。
...
- 至少实现一个 “Attention view”：从事件流派生出 `NeedApproval/Failed/Stuck/...` 的可查询状态，并附带可行动的语义标记（例如 `test_failed` marker；TODO）。
...
- 执行后端：统一走一个 “可拦截 execve 的 shell/runner”（不要直接 `Command::new("bash -lc ...")` 然后祈祷安全；TODO 草案见 `docs/execve_wrapper.md`）。
