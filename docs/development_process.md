...
2. **注意力队列（Attention / Inbox）**：把“需要人介入”的点变成可枚举状态。v0.2.0 已实现的 `attention_state` 见 `docs/attention.md`；`thread/attention` 现已支持显式 marker 事件（`AttentionMarkerSet`，并对 `test_failed` 支持 `AttentionMarkerCleared`）并保留历史线程推断回退。
3. **可暂停/可打断/可步进**：至少要能 `pause/resume/interrupt/cancel`；更进一步要支持 turn 级 step（默认 plan → approve → act）。
4. **workspace 生命周期脚本化**：`setup/run/archive(or teardown)` 必须是约定俗成的 hook（参考 `docs/research/onecode.md`、`docs/research/superset.md`；v0.2.0 最小执行入口见 `docs/workspace_hooks.md`）。
...
- 至少实现一个 “Attention view”：从事件流派生出 `NeedApproval/Failed/Stuck/...` 的可查询状态，并附带可行动的语义标记（`plan_ready/diff_ready/test_failed` 已支持显式事件优先聚合，并保留推断回退）。
...
- 执行后端：统一走一个 “可拦截 execve 的 shell/runner”（不要直接 `Command::new("bash -lc ...")` 然后祈祷安全；v0.2.x 最小链路已落地，后续扩展见 `docs/execve_wrapper.md`）。
