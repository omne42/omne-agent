...
数据模型：`omne_agent_protocol::ArtifactMetadata`（详见 `crates/agent-protocol/src/lib.rs`）。
...
- bounded history 默认关闭：不会保留旧版本内容（只有 version 号递增）；可用 `OMNE_AGENT_ARTIFACT_HISTORY_MAX_VERSIONS` 启用（见 §4）。
...
```bash
omne-agent artifact list <thread_id>
omne-agent artifact read <thread_id> <artifact_id>
omne-agent artifact delete <thread_id> <artifact_id>
```

生成 diff artifact（`git diff`，写入 user artifact `artifact_type="diff"`）：

```bash
omne-agent thread diff <thread_id> --max-bytes 4194304 --wait-seconds 30
```

生成 patch artifact（`git diff --binary`，写入 user artifact `artifact_type="patch"`）：

```bash
omne-agent thread patch <thread_id> --max-bytes 4194304 --wait-seconds 30
```
...
- rotate 默认阈值 `8MiB`，可用 `OMNE_AGENT_PROCESS_LOG_MAX_BYTES_PER_PART` 覆盖
...
- `omne_agent_protocol::ArtifactPreviewKind`：
...
- `omne_agent_protocol::ArtifactMetadata.preview: Option<ArtifactPreview>`（旧的 `*.metadata.json` 可能缺失该字段）
...
- `OMNE_AGENT_ARTIFACT_HISTORY_MAX_VERSIONS`：
  - `0` = 关闭（默认）
  - `N>0` = 保留最近 N 个旧版本
...
```bash
omne-agent artifact versions <thread_id> <artifact_id>
omne-agent artifact read <thread_id> <artifact_id> --version 2
```
...
- 开启 `OMNE_AGENT_ARTIFACT_HISTORY_MAX_VERSIONS=2`：
  - 连续写入同一 `artifact_id` 3 次后，`history/<artifact_id>/` 下最多只保留 2 个旧版本文件。
- 执行 `omne-agent artifact delete <thread_id> <artifact_id>` 后，`history/<artifact_id>/` 会被级联删除。
