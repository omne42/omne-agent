...
- app-server：`omne-agent-app-server --execpolicy-rules <PATH> [--execpolicy-rules <PATH> ...]`
- `omne-agent` CLI：同名参数透传给 app-server：`omne-agent --execpolicy-rules <PATH> ...`
...
- 配置位置：`./.omne_agent_data/spec/modes.yaml` → `modes.<name>.permissions.command.execpolicy_rules: [<path>...]`
...
```bash
cargo run -p omne-agent-execpolicy -- check -r /path/to/policy.rules -- git status
```

让 `omne-agent` 在启动 app-server 时加载规则：

```bash
cargo run -p omne-agent -- --execpolicy-rules /path/to/policy.rules thread start --json
```
...
- `omne-agent thread config-explain <thread_id>` 能解释：当前有效的 rules 来源顺序（global/mode/thread）与每次 `process/start` 的 matched rules。
