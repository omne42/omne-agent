...
- 不做隔离 workspace 的自动 apply 回填/冲突处理（`isolated_write` 已提供 patch handoff，可人工回填）。
...
补充：控制面 `thread/fork` 已支持可选 `cwd` 覆盖。`agent_spawn(workspace_mode=isolated_write)` 会利用这个能力把子线程落到隔离副本目录，而不是父线程共享目录。

当前建议的安全边界：

- 把 fork/agent_spawn 当作 **并发只读分析**（read-only tools）是最稳妥默认。
- 真要并发写代码：使用 `workspace_mode=isolated_write`（已实现最小隔离副本 + patch handoff）；自动 apply/冲突处理仍是后续 TODO（见 `docs/workspace_hooks.md` 与 parity 的 scheduler TODO）。
...
3. 按 `spawn_mode` 为每个 task 创建子 thread：
   ...
   - 当 `workspace_mode=isolated_write` 时，先复制父 workspace 到隔离目录：`<server.cwd>/.omne_data/tmp/subagents/<parent_thread_id>/<task-id>-<nonce>/repo`，再以该目录作为子线程 `cwd`
...
6. 当子 thread 的该 turn 结束（`TurnCompleted`）时，写出一个 user artifact 作为结果锚点（默认 `artifact_type="fan_out_result"`）
   ...
   - 当 `workspace_mode=isolated_write` 且 workspace 有改动时，会额外写 `artifact_type="patch"`，并在 `fan_out_result.isolated_write_patch` 返回 `artifact_id/read_cmd/truncated`。
...
- [x] `workspace_mode=isolated_write` 已实现最小隔离副本（独立 `cwd` + `workspace_write`），不做隐式降级到共享写。
- [x] `workspace_mode=isolated_write` 会把 patch handoff 回填到 `fan_out_result`（`isolated_write_patch` / `isolated_write_handoff`）；自动 apply 与冲突处理仍未实现。
...
- [x] workflow fan-out：当依赖任务失败/中断/取消/stuck 时，后继任务会被标记 `Cancelled` + `dependency_blocked=true`，不再启动。
...
- **C：隔离 workspace（可写 fan-out）**：已落地“最小隔离副本执行 + patch handoff”；后续需补“自动 apply patch / 冲突处理”。
