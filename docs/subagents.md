...
注意：`omne thread spawn` 会保持 RPC 连接直到子 turn 完成（尤其是 stdio 非 daemon 场景），避免 CLI 退出导致 `omne-app-server` 结束从而取消后台 turn，造成只写入 `TurnStarted` 却没有 `TurnCompleted` 的“半截线程”。
...
3. 按 `spawn_mode` 为每个 task 创建子 thread：
   ...
   - 当 `workspace_mode=isolated_write` 时，隔离目录固定为：`<server.cwd>/.omne_data/tmp/subagents/<parent_thread_id>/<task-id>-<nonce>/repo`
