...
- 不做完整 scheduler（队列/依赖/抢占/公平调度/工作区隔离等）；但已实现最小的全局 LLM 并发限流与优先级（见 `OMNE_AGENT_MAX_CONCURRENT_LLM_REQUESTS`/`OMNE_AGENT_LLM_FOREGROUND_RESERVE`）。
...
补充：CLI 也提供了等价的便捷命令 `omne-agent thread spawn <thread_id> "<input>" [--model ...] [--openai-base-url ...]`，其实现同样是 `thread/fork` +（可选）`thread/configure` + `turn/start`（对照：`crates/agent-cli/src/main/process_and_utils.rs`）。
...
2. `guard`：并发上限 `OMNE_AGENT_MAX_CONCURRENT_SUBAGENTS`（默认 `4`；`0` 表示不限制）
3. 按 `spawn_mode` 为每个 task 创建子 thread：
   - `fork`：`thread/fork`
   - `new`：`thread/start`（cwd 继承父 thread）
4. 对子 thread 执行 `thread/configure`：默认设置 `sandbox_policy=read_only` + `mode=<child_mode>`（并可设置 `model/openai_base_url`）
5. 按依赖关系启动子任务：依赖已满足的 task 先启动；其余等待依赖完成后再启动（后台执行）
   - turn priority：subagent turns 使用 `priority=background`；全局 LLM worker pool 会优先保障 foreground（见 `OMNE_AGENT_MAX_CONCURRENT_LLM_REQUESTS`/`OMNE_AGENT_LLM_FOREGROUND_RESERVE`）。
6. 当子 thread 的该 turn 结束（`TurnCompleted`）时，写出一个 user artifact 作为结果锚点（默认 `artifact_type="fan_out_result"`）

返回值（现状）：

- `tasks[]`：每个 task 的 `thread_id/turn_id/log_path/last_seq/status` 等运行句柄（`turn_id` 可能为 `null`，表示尚未启动）

可观测性：

- forked thread 会出现在 `thread/list_meta` 与 `omne-agent inbox --watch` 中；可用 `thread/subscribe` 重放/追踪事件（见 `docs/thread_event_model.md`）。
...
- 汇总必须带 provenance：引用哪些 threads/turns/artifacts（见 `omne_agent_protocol::ArtifactProvenance`）。
...
- fan-out 产生的每个子任务都能在 `omne-agent inbox` 里定位到 `thread_id/turn_id`，并可用 `thread/subscribe` 重放事件。
...
- [x] workflow fan-out：`omne-agent command run <name> --fan-out` 解析 `## Task:`，并在父 thread 写入 `artifact_type="fan_in_summary"` 汇总 artifact（用于主 turn 上下文/可追溯）。

### 3.3.2 方案选型（A/B/C，避免“边写边发明”）

- **A：客户端编排（只读 fan-out）**：复用 `thread/fork + agent_spawn`，子任务只读并写 `fan_out_result`，fan-in 由 orchestrator 写 `fan_in_summary`；审批/输入不做桥接（依赖 `omne-agent inbox` 聚合）。
...
omne-agent command run <name> --fan-out

# 低阶原语：只 fork + 启动子 turn（不做 fan-in 汇总）
omne-agent thread spawn <thread_id> "<input>"
```
