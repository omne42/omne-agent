...
fork 会创建一个新 thread，并把部分事件复制到新 thread 的事件流：
...
- 当 `thread/fork` 未指定 `cwd` 时，`ThreadConfigUpdated` 会按原样复制。
- 当 `thread/fork --cwd <new-root>` 切到新的 workspace 时，workspace-scoped 配置会随新根重定位：
  - `sandbox_writable_roots` 中位于父 workspace 下的绝对路径会 rebased 到子 workspace 对应位置。
  - `execpolicy_rules` 中位于父 workspace 下的绝对路径也会 rebased 到子 workspace 对应位置。
  - 相对 `execpolicy_rules` 保持相对路径文本，因此会按子 thread 的 `cwd` 重新解析。
- 显式指向 workspace 外部的绝对路径不会被改写。

### 1.2 fork 不会复制什么（危险点）

- ❌ 不复制 `AttentionMarkerSet/Cleared`（避免把 `artifact_id` / `process_id` 一类引用带到 forked thread 后变成悬空指针）
...
- fork 的语义更像“复制对话/审批/结论”，不是“复制完整可回放上下文”（tool/process/artifact/checkpoint/attention 历史都会缺失）。
...
进一步说，`thread/fork --cwd` 不只是切换目录；它也会把 workspace-scoped 的 thread 配置一起 rebased，避免子线程继续沿用父 workspace 的写根或 workspace 内绝对 execpolicy 路径。
