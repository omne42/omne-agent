...
- 不做完整 scheduler（队列/依赖/抢占/公平调度/工作区隔离等）；但已实现最小的全局 LLM 并发限流与优先级（见 `CODE_PM_MAX_CONCURRENT_LLM_REQUESTS`/`CODE_PM_LLM_FOREGROUND_RESERVE`）。
...
5. 按依赖关系启动子任务：依赖已满足的 task 先启动；其余等待依赖完成后再启动（后台执行）
   - turn priority：subagent turns 使用 `priority=background`；全局 LLM worker pool 会优先保障 foreground（见 `CODE_PM_MAX_CONCURRENT_LLM_REQUESTS`/`CODE_PM_LLM_FOREGROUND_RESERVE`）。
