...
- 不做完整 scheduler（队列/依赖/抢占/公平调度/工作区隔离等）；但已实现最小的全局 LLM 并发限流与优先级（见 `OMNE_MAX_CONCURRENT_LLM_REQUESTS`/`OMNE_LLM_FOREGROUND_RESERVE`）。
...
2. `guard`：并发上限 `OMNE_MAX_CONCURRENT_SUBAGENTS`（默认 `4`；`0` 表示不限制）
...
- **A：客户端编排（只读 fan-out）**：复用 `thread/fork + agent_spawn`，子任务只读并写 `fan_out_result`，fan-in 由 orchestrator 写 `fan_in_summary`；审批/输入不做桥接（依赖 `omne inbox` 聚合）。
