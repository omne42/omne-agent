...
| `sandbox_network_access` | `deny` / `allow` | 否 | 控制命令执行链路里的“潜在联网命令”检测与阻断，不是内核级网络沙箱。 |
...
| `sandbox_network_access` | `deny` / `allow` | `deny` | 控制命令类网络访问的 best-effort argv 检测（`process/start`、`execve gate`、`mcp/call`）。它会阻断明显或潜在联网的命令入口，但不是内核级网络隔离，也不直接限制 `web/*` 工具。 |
