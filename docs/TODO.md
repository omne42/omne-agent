...
- 提供 Node 侧入口（npm 安装/IDE/GUI 集成）来启动并驱动 `omne-app-server`。
...
- [ ] 生成并发布协议 TS types（单一真源：`crates/app-server-protocol`）
  ...
  - [ ] 产物以 Node package 形态发布（例如 `packages/omne-app-server-protocol`），避免 consumers 依赖 Rust toolchain
...
- **GUI**：放在 Node/TypeScript（未来 `packages/gui` 或独立 repo），thin client，通过 `omne-app-server` 协议驱动；不要复制任何 core 语义到前端。
